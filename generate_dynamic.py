...
def generate_docs(row):
    try:
        ...
        return pdf_path, file_name_with_date
    except Exception as e:
         logging.error(f"❌ Error occured in generating a Document")
def sending_email_to_receipts(row, pdf_path, file_name_with_date):
    name = row.get("Name", f"Record_{row.name}")
    # load body
    body=''
    try:
...
for _, row in df.iterrows():
    pdf_path, file_name_with_date = generate_docs(row)
    # sending email
    sending_email_to_receipts(row, pdf_path, file_name_with_date)
